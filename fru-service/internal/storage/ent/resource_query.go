...
func (rq *ResourceQuery) QueryLabels() *LabelQuery {
	query := (&LabelClient{config: rq.config}).Query()
...
func (rq *ResourceQuery) QueryAnnotations() *AnnotationQuery {
	query := (&AnnotationClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *ResourceQuery) Clone() *ResourceQuery {
	...
	return &ResourceQuery{
		...
		withAnnotations: rq.withAnnotations.Clone(),
...
func (rq *ResourceQuery) WithLabels(opts ...func(*LabelQuery)) *ResourceQuery {
	query := (&LabelClient{config: rq.config}).Query()
...
func (rq *ResourceQuery) WithAnnotations(opts ...func(*AnnotationQuery)) *ResourceQuery {
	query := (&AnnotationClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withAnnotations = query
	return rq
}
...
func (rq *ResourceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Resource, error) {
	...
	if query := rq.withLabels; query != nil {
		if err := rq.loadLabels(ctx, query, nodes,
			func(n *Resource) { n.Edges.Labels = []*Label{} },
			func(n *Resource, e *Label) { n.Edges.Labels = append(n.Edges.Labels, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withAnnotations; query != nil {
		if err := rq.loadAnnotations(ctx, query, nodes,
...
func (rq *ResourceQuery) loadLabels(ctx context.Context, query *LabelQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *Label)) error {
...
func (rq *ResourceQuery) loadAnnotations(ctx context.Context, query *AnnotationQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *Annotation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Resource)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
